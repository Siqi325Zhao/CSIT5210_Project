...
from func import preprocess_data_real_time
import hydra
import numpy as np
...
def main(cfg: DictConfig) -> None:
    ...
    print("sliceing_length: ", sliceing_length)
    head = str(sliceing_length) + head

    for consumer_record in consumer:
        output = consumer_record.value
        data_stored.append(output.decode('utf-8'))
        index += 1

        path / f"data_{iteration}.txt"

        if index == sliceing_length:
            iteration += 1
            print(head)
            data_stored.insert(0, head)

            # _path = path + "data%d.txt" % iteration
            # _path = path / f"data_{iteration}.txt"

            # with open('data_%d.txt' % iteration, 'w') as f:
            with open(path / f"data_{iteration}.txt", 'w') as f:
                for item in data_stored:
                    f.write("%s" % item)
            index = 0
            data_stored = []
        else:
        ...
        for k in K_list:
            ...
            for s in np.arange(cfg.setting.n_seeds):
                rel_mat_true, rel_mat_obs, info = preprocess_data_real_time(
                ...
                print("rel_mat_obs: ", rel_mat_obs.shape)
                print(rel_mat_obs)
                ...
                for alpha_ in alpha_list:
                    pi_nsw = compute_pi_nsw(
                        rel_mat=rel_mat_obs, v=v, alpha=alpha_)
